...
KGI_ACTIVE_WARRANTS_CACHE_PATH = os.path.join(CACHE_DIR, "kgi_active_warrants_today_cache.csv")
...
# 這張榜單不追蹤特定分點，也不顯示分點。
# 它會在今日成交量過濾階段，用 API4 找出今日有成交權證與當日券商代號，
# 再用 API5 回查每組「權證 × 券商」當日買進金額 / 賣出金額，
# 依標的股彙總：同一標的底下所有認購權證的「正買超金額」加總。
...
# 今日有成交量權證來源：
# kgi：使用凱基權證搜尋器 / 匯出資料作為唯一母體，不再用 ISIN 全市場清單 + MoneyDJ API4 掃全部權證。
# moneydj_api4：保留舊流程備援，平常不要使用。
ACTIVE_WARRANT_SOURCE = os.getenv("ACTIVE_WARRANT_SOURCE", "kgi").strip().lower()
KGI_BASE_PAGE = "https://warrant.kgi.com/EDWebSite/Views/StrategyCandidate/MarketStatistics.aspx"
KGI_SEARCH_PAGE = "https://warrant.kgi.com/EDWebSite/Views/WarrantSearch/WarrantSearch.aspx"
KGI_API_URL = "https://warrant.kgi.com/EDWebService/WSInterfaceSwap.asmx/GetService"
KGI_ACTIVE_WARRANT_CSV = os.getenv("KGI_ACTIVE_WARRANT_CSV", "").strip()
KGI_ACTIVE_MIN_COUNT = int(os.getenv("KGI_ACTIVE_MIN_COUNT", "1"))
KGI_CP = os.getenv("KGI_CP", "ALL").strip() or "ALL"
# 若凱基不支援直接全市場查詢，預設不回退到成交金額前十大標的，避免資料不完整卻誤以為全市場。
KGI_ALLOW_TOP_UNDERLYING_FALLBACK = os.getenv("KGI_ALLOW_TOP_UNDERLYING_FALLBACK", "0").strip().lower() in ("1", "true", "yes")
KGI_TOP20_CACHE_VERSION = "kgi_active_api5_v3"
KGI_TOP20_WORKERS = int(os.getenv("KGI_TOP20_WORKERS", str(max(10, min(MAX_WORKERS, 40)))))
FULL_MARKET_TOP20_API5_WORKERS = int(os.getenv("FULL_MARKET_TOP20_API5_WORKERS", str(MAX_WORKERS)))
FULL_MARKET_UNDERLYING_TOP20_CACHE_VERSION = KGI_TOP20_CACHE_VERSION
...
CACHE_SHEET_NAME_MAP = {
    ...
    "kgi_active_warrants_today_cache.csv": "快取_KGI今日有成交權證",
}
...
def _kgi_clean_warrant_code(value):
    s = str(value).strip()
    if s.endswith(".0"):
        s = s[:-2]
    s = "".join(ch for ch in s if ch.isdigit())
...
def _kgi_clean_underlying_code(value):
    s = str(value).strip()
    if s.endswith(".0"):
        s = s[:-2]
    s = s.replace(" ", "").replace("\u3000", "")
...
def _kgi_detect_col(df, keywords):
    ...
    for keyword in keywords:
        ...
        if keyword in upper_cols:
            return upper_cols[keyword]

    for c in df.columns:
        c_upper = str(c).upper()
        if any(str(keyword).upper() in c_upper for keyword in keywords):
            return c

    return None


def _kgi_extract_json_text(raw_text):
    raw_text = str(raw_text or "")

    for tag in ["ValueOfJson", "Data"]:
        match = re.search(rf"<{tag}>(.*?)</{tag}>", raw_text, re.DOTALL)
        if match:
            txt = html.unescape(match.group(1)).strip()
            if txt:
                return txt
...
def _kgi_post(session, service_id, params, referer=None, retry=2):
    referer = referer or KGI_SEARCH_PAGE
    headers = {
    ...
    payload = {
        ...
        "parametersOfJson": json.dumps(params, ensure_ascii=False),
    }

    last_error = None
    for attempt in range(1, retry + 2):
        try:
            res = session.post(KGI_API_URL, headers=headers, data=payload, timeout=(8, 30))
            res.raise_for_status()
            json_text = _kgi_extract_json_text(res.text)
            if not json_text:
                raise RuntimeError(f"找不到 JSON 內容，回傳前 300 字：{res.text[:300]}")
            ...
            last_error = e
            if attempt <= retry:
                time.sleep(min(10, attempt * 1.2 + random.uniform(0.1, 0.8)))

    print(f"  ⚠️ KGI API 呼叫失敗：{service_id}，原因：{type(last_error).__name__}: {last_error}")
    return []


def _kgi_init_session():
    session = requests.Session()
    session.headers.update({"User-Agent": HDR.get("User-Agent", "Mozilla/5.0")})
    try:
        r = session.get(KGI_BASE_PAGE, headers={"User-Agent": HDR.get("User-Agent", "Mozilla/5.0")}, timeout=(8, 20))
        print(f"  KGI MarketStatistics HTTP {r.status_code}")
    except Exception as e:
        print(f"  ⚠️ KGI MarketStatistics 初始化失敗：{type(e).__name__}: {e}")
    try:
        r = session.get(KGI_SEARCH_PAGE, headers={"User-Agent": HDR.get("User-Agent", "Mozilla/5.0")}, timeout=(8, 20))
        print(f"  KGI WarrantSearch HTTP {r.status_code}")
    except Exception as e:
        print(f"  ⚠️ KGI WarrantSearch 初始化失敗：{type(e).__name__}: {e}")
    return session


def _kgi_build_search_params(cp="ALL", volume=1, underlying_value=None, omit_underlying=False):
    params = {
    ...
    if not omit_underlying:
        params["UND_INSTR_INSNBR"] = underlying_value

    return params


def _kgi_full_market_trials():
    cp = KGI_CP
    return [
        ("不帶 UND_INSTR_INSNBR", _kgi_build_search_params(cp=cp, volume=1, omit_underlying=True)),
        ("UND_INSTR_INSNBR=ALL", _kgi_build_search_params(cp=cp, volume=1, underlying_value="ALL")),
        ("UND_INSTR_INSNBR=-1", _kgi_build_search_params(cp=cp, volume=1, underlying_value="-1")),
        ("UND_INSTR_INSNBR=0", _kgi_build_search_params(cp=cp, volume=1, underlying_value="0")),
        ("UND_INSTR_INSNBR=空字串", _kgi_build_search_params(cp=cp, volume=1, underlying_value="")),
        ("UND_INSTR_INSNBR=null", _kgi_build_search_params(cp=cp, volume=1, underlying_value=None)),
    ]


def _kgi_standardize_active_warrant_df(df):
    if df is None or df.empty:
        return []

    df = df.copy().fillna("")
    code_col = _kgi_detect_col(df, ["INSTR_STKID", "STKID", "STOCK_ID", "INSWRT_ID", "WRT_ID", "權證代號", "代號"])
    name_col = _kgi_detect_col(df, ["INSTR_NAME", "STOCK_NAME", "INSWRT_NAME", "WARRANT_NAME", "權證名稱", "名稱"])
    volume_col = _kgi_detect_col(df, ["VOLUME", "MKTDAT_VOLUME", "成交量", "成交張數"])
    und_code_col = _kgi_detect_col(df, ["UND_INSTR_STKID", "UND_STKID", "UNDERLYING_STKID", "標的代號"])
    und_name_col = _kgi_detect_col(df, ["UND_INSTR_NAME", "UND_NAME", "UNDERLYING_NAME", "標的名稱"])

    if not code_col or not name_col:
        print(f"  ⚠️ KGI 欄位不足，無法解析權證代號 / 名稱。欄位：{list(df.columns)}")
        return []
    ...
    for _, r in df.iterrows():
        code = _kgi_clean_warrant_code(r.get(code_col, ""))
        name = str(r.get(name_col, "")).strip()
        volume = _kgi_clean_number(r.get(volume_col, 0)) if volume_col else 0.0
        ...
        if "購" not in name:
            continue
        if re.search(r"售|牛|熊", name):
            continue
        if code in seen:
            continue

        underlying_code = _kgi_clean_underlying_code(r.get(und_code_col, "")) if und_code_col else ""
        underlying_name = str(r.get(und_name_col, "")).strip() if und_name_col else ""
        ...
        rows.append({
            ...
            "標的股": underlying_code,
            "標的名稱": underlying_name,
            "KGI成交量": int(round(volume)),
        })

    return rows


def load_kgi_active_warrants_cache():
    if ACTIVE_WARRANT_FORCE_REFRESH:
        return []

    df = read_cache_csv(KGI_ACTIVE_WARRANTS_CACHE_PATH)
    if df.empty:
        return []

    required_cols = ["執行日期", "權證代號", "權證名稱", "標的股", "標的名稱"]
    for col in required_cols:
        if col not in df.columns:
            return []
    ...
    seen = set()
    for _, r in df.iterrows():
        code = _kgi_clean_warrant_code(r.get("權證代號", ""))
        name = str(r.get("權證名稱", "")).strip()
        ...
        warrants.append({
            "代號": code,
            "名稱": name,
            "標的股": _kgi_clean_underlying_code(r.get("標的股", "")),
            "標的名稱": str(r.get("標的名稱", "")).strip(),
            "KGI成交量": int(_kgi_clean_number(r.get("KGI成交量", 0))),
        })

    if warrants:
        print(f"  ♻️ 已讀取 KGI 今日有成交權證快取：{len(warrants):,} 支")

    return warrants


def save_kgi_active_warrants_cache(warrants):
    if not USE_CACHE or not warrants:
        return
    ...
    for w in warrants:
        rows.append({
            ...
            "KGI成交量": int(_kgi_clean_number(w.get("KGI成交量", 0))),
        })

    df = pd.DataFrame(rows, columns=["執行日期", "權證代號", "權證名稱", "標的股", "標的名稱", "KGI成交量"])
    write_cache_csv(df, KGI_ACTIVE_WARRANTS_CACHE_PATH)
    print(f"  💾 已更新 KGI 今日有成交權證快取：{KGI_ACTIVE_WARRANTS_CACHE_PATH}")


def _find_today_kgi_csv_path():
    if KGI_ACTIVE_WARRANT_CSV:
        return KGI_ACTIVE_WARRANT_CSV if os.path.exists(KGI_ACTIVE_WARRANT_CSV) else ""

    today = datetime.today().strftime("%Y%m%d")
    candidates = [
        os.path.join(OUTPUT_DIR, f"KGI_全市場今日成交量大於0認購權證_{today}.csv"),
        os.path.join(OUTPUT_DIR, f"KGI_今日成交量大於0認購權證_{today}.csv"),
        os.path.join(os.getcwd(), f"KGI_全市場今日成交量大於0認購權證_{today}.csv"),
        os.path.join(os.getcwd(), f"KGI_今日成交量大於0認購權證_{today}.csv"),
    ]
...
def load_kgi_active_warrants_from_csv(path):
    try:
        ...
        try:
            ...
            print(f"  ⚠️ KGI CSV 讀取失敗：{path}，原因：{type(e).__name__}: {e}")
            return []

    warrants = _kgi_standardize_active_warrant_df(df)
    if warrants:
        print(f"  ✅ 已從 KGI CSV 讀取今日有成交認購權證：{len(warrants):,} 支，來源：{path}")
    return warrants


def fetch_kgi_active_warrants_live():
    print("【Step 1】使用 KGI 抓今日成交量 > 0 的認購權證清單...")

    csv_path = _find_today_kgi_csv_path()
    if csv_path:
        warrants = load_kgi_active_warrants_from_csv(csv_path)
        if warrants:
            save_kgi_active_warrants_cache(warrants)
            return warrants

    session = _kgi_init_session()
    best_label = ""
    best_rows = []

    print("  🔎 嘗試 KGI S0600013_GetWarrants 全市場成交量 > 0 參數...")
    for label, params in _kgi_full_market_trials():
        data = _kgi_post(session, "S0600013_GetWarrants", params, referer=KGI_SEARCH_PAGE, retry=1)
        df = pd.DataFrame(data)
        warrants = _kgi_standardize_active_warrant_df(df)
        print(f"    {label}：原始 {len(df):,} 筆，認購成交量>0 {len(warrants):,} 支")
        if len(warrants) > len(best_rows):
            best_rows = warrants
            best_label = label

    if best_rows and len(best_rows) >= KGI_ACTIVE_MIN_COUNT:
        print(f"  ✅ KGI 全市場今日有成交認購權證取得完成：{len(best_rows):,} 支，參數：{best_label}")
        save_kgi_active_warrants_cache(best_rows)
        return best_rows

    if KGI_ALLOW_TOP_UNDERLYING_FALLBACK:
        print("  ⚠️ KGI 全市場查詢失敗，KGI_ALLOW_TOP_UNDERLYING_FALLBACK=1，改用成交金額前段標的補抓。此模式不是完整全市場。")
        top_params = {
            "MKTDAT_CP": KGI_CP,
            "LocationPathName": "/EDWebSite/Views/StrategyCandidate/MarketStatistics.aspx",
        }
        top_data = _kgi_post(session, "S0600015_GetRankInFrontByDealAmountForUnderlying", top_params, referer=KGI_BASE_PAGE, retry=1)
        top_df = pd.DataFrame(top_data).fillna("")
        if "UND_INSTR_INSNBR" in top_df.columns:
            rows = []
            for uid in top_df["UND_INSTR_INSNBR"].dropna().astype(str).drop_duplicates().tolist():
                params = _kgi_build_search_params(cp=KGI_CP, volume=1, underlying_value=uid)
                data = _kgi_post(session, "S0600013_GetWarrants", params, referer=KGI_SEARCH_PAGE, retry=1)
                rows.extend(data)
                time.sleep(0.35 + random.uniform(0.05, 0.25))
            warrants = _kgi_standardize_active_warrant_df(pd.DataFrame(rows))
            if warrants:
                print(f"  ✅ KGI 前段標的補抓完成：{len(warrants):,} 支（注意：非完整全市場）")
                save_kgi_active_warrants_cache(warrants)
                return warrants

    print("  ❌ KGI 今日成交量 > 0 認購權證取得失敗，且不回退 ISIN / MoneyDJ 全市場掃描。")
    print("  請確認 KGI 權證搜尋器實際的全市場成交量>0 API，或設定 KGI_ACTIVE_WARRANT_CSV 指向你已匯出的 KGI 今日有成交權證 CSV。")
    return []


def get_active_call_warrants_from_kgi():
    global ACTIVE_WARRANT_LATEST_TRADE_DATE, ACTIVE_WARRANT_FILTER_SOURCE

    cached = load_kgi_active_warrants_cache()
    if cached:
        ACTIVE_WARRANT_LATEST_TRADE_DATE = active_warrant_cache_run_date()
        ACTIVE_WARRANT_FILTER_SOURCE = "kgi_cache"
        return cached

    warrants = fetch_kgi_active_warrants_live()
    ACTIVE_WARRANT_LATEST_TRADE_DATE = active_warrant_cache_run_date() if warrants else ""
    ACTIVE_WARRANT_FILTER_SOURCE = "kgi" if warrants else "kgi_empty"
    return warrants


def _api5_today_trade_values(warrant_code, broker_code, target_date):
...
def build_full_market_underlying_top20_from_kgi_active_warrants(warrants):
    """
    KGI 模式的 TOP20 建立：
    1. KGI 先提供今日有成交量的認購權證母體。
    2. 只對這批權證用 MoneyDJ API4 找當日有交易的所有券商分點。
    3. 再用 API5 回查該權證 × 分點當日買進 / 賣出金額。
    4. 不套 TARGET_PATTERNS，不追蹤特定分點，只依標的彙總正買超。
    """
    ...
    if not warrants:
        FULL_MARKET_UNDERLYING_TOP20_ROWS = []
        FULL_MARKET_UNDERLYING_TOP20_DATE = ""
        FULL_MARKET_UNDERLYING_TOP20_SOURCE = "kgi_empty"
        return
    ...
    print("【Step 1b】KGI 今日有成交權證 → 建立全市場標的買超 TOP20（API5 金額校正）...")
    print(f"  🔎 僅掃 KGI 今日有成交權證：{len(warrants):,} 支，區間 {start_s} ~ {end_s}")
    ...
    def scan_one(w):
        ...
        for row in rows:
            ...
            if not date_str:
                continue
            bcode = str(row.get("V2", "")).strip()
            bname = str(row.get("V3", "")).strip()
            if not bcode:
                continue
            out.append({
                "日期": normalize_date_str(date_str),
                "權證代號": code,
                "權證名稱": str(w.get("名稱", "")).strip(),
                "標的股": str(w.get("標的股", "")).strip(),
                "標的名稱": str(w.get("標的名稱", "")).strip(),
    ...
    with ThreadPoolExecutor(max_workers=ACTIVE_WARRANT_WORKERS) as ex:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                rows = []
            for rec in rows:
                date_counter[rec["日期"]] = date_counter.get(rec["日期"], 0) + 1
                key = (rec["日期"], rec["權證代號"], rec["券商代號"])
                pair_map[key] = rec
            if done % 500 == 0:
                print(f"  [{done:,}/{len(warrants):,}] KGI active API4 掃描中，分點組合 {len(pair_map):,} 組...")

    if not date_counter:
        print("  ⚠️ KGI active 權證未從 API4 掃到當日分點資料，TOP20 略過。")
        FULL_MARKET_UNDERLYING_TOP20_ROWS = []
        FULL_MARKET_UNDERLYING_TOP20_DATE = ""
        FULL_MARKET_UNDERLYING_TOP20_SOURCE = "kgi_api4_empty"
        return

    latest_trade_date = sorted(date_counter.keys(), key=lambda d: parse_date(d) or datetime.min)[-1]
    pairs = [rec for key, rec in pair_map.items() if key[0] == latest_trade_date]
    print(f"  ✅ 最新交易日：{latest_trade_date}，需 API5 校正分點組合：{len(pairs):,} 組")

    ranking_map = {}
    done = 0

    def fetch_pair(rec):
        trade = _api5_today_trade_values(rec["權證代號"], rec["券商代號"], latest_trade_date)
        return rec, trade

    with ThreadPoolExecutor(max_workers=KGI_TOP20_WORKERS) as ex:
        futures = {ex.submit(fetch_pair, rec): rec for rec in pairs}
        for future in as_completed(futures):
            done += 1
            try:
                rec, trade = future.result()
            except Exception:
                rec, trade = {}, None
            ...
            warrant_key = f'{rec.get("權證代號", "")} {rec.get("權證名稱", "")}'.strip()
            broker_key = f'{rec.get("券商名稱", "")}({rec.get("券商代號", "")})'.strip()
            row["權證代號集合"].add(rec.get("權證代號", ""))
            row["權證買超"][warrant_key] = row["權證買超"].get(warrant_key, 0) + int(amount_to_add)
            row["分點買超"][broker_key] = row["分點買超"].get(broker_key, 0) + int(amount_to_add)

            if done % 500 == 0:
                print(f"  [{done:,}/{len(pairs):,}] API5 校正 TOP20 中...")
    ...
    for rec in ranking_map.values():
        ...
        top_broker_text = "；".join(
            f"{name}:{fmt_amount(amount)}"
            for name, amount in sorted(rec["分點買超"].items(), key=lambda x: x[1], reverse=True)[:3]
            if amount > 0
        )
        rows.append({
            "日期": rec["日期"],
    ...
    FULL_MARKET_UNDERLYING_TOP20_ROWS = rows
    FULL_MARKET_UNDERLYING_TOP20_DATE = latest_trade_date
    FULL_MARKET_UNDERLYING_TOP20_SOURCE = "kgi_active_api5"
    save_full_market_underlying_top20_cache(rows, latest_trade_date)
    print(f"  ✅ KGI active 全市場標的買超 TOP20 建立完成：{len(rows):,} 名")
...
def load_full_market_underlying_top20_cache():
    ...
    required_cols = ["執行日期", "資料來源版本", "日期", "排名", "標的股", "標的名稱", "累積買超金額"]
    for col in required_cols:
        if col not in df.columns:
            # 舊版快取是由 API4 金額欄位估算而來，容易出現金額單位錯誤。
            # 若沒有資料來源版本欄位，直接視為舊快取，不再沿用。
            return [], ""
    ...
    df = df[df["資料來源版本"].astype(str).str.strip() == FULL_MARKET_UNDERLYING_TOP20_CACHE_VERSION].copy()
    if df.empty:
        return [], ""
    ...
    if rows:
        print(
            f"  ♻️ 已讀取全市場標的買超 TOP20 快取：{len(rows):,} 筆，"
            f"日期：{target_date}，版本：{FULL_MARKET_UNDERLYING_TOP20_CACHE_VERSION}"
        )
...
def save_full_market_underlying_top20_cache(rows, target_date):
    ...
    for row in rows:
        out = {
            "執行日期": run_date,
            "資料來源版本": FULL_MARKET_UNDERLYING_TOP20_CACHE_VERSION,
    ...
    print(
        f"  💾 已更新全市場標的買超 TOP20 快取：{FULL_MARKET_UNDERLYING_TOP20_CACHE_PATH} "
        f"版本：{FULL_MARKET_UNDERLYING_TOP20_CACHE_VERSION}"
    )

def _safe_int_from_api_value(value, multiplier=1):
    try:
        s = str(value).replace(",", "").replace("--", "").replace("-", "").strip()
        if not s or s in ("None", "nan", "null"):
            return 0
...
def extract_api5_trade_values_for_date(rows, target_date):
    """
    從 API5 的單一權證 + 單一券商歷史資料中，取出指定日期買賣超。
    ...
    修正原因：
    API4 適合判斷今日是否有成交與列出分點，但 API4 的金額欄位順序 / 單位不穩，
    先前直接用 API4 建立 TOP20 會出現台灣大等標的金額被放大、平均權證價格不合理的問題。
    """
    target_date = normalize_date_str(target_date)
    ...
    for row in rows:
        ...
        buy_s = _safe_int_from_api_value(row.get("V2", 0), multiplier=1)
        sell_s = _safe_int_from_api_value(row.get("V3", 0), multiplier=1)
        buy_a = _safe_int_from_api_value(row.get("V4", 0), multiplier=1000)
        sell_a = _safe_int_from_api_value(row.get("V5", 0), multiplier=1000)
...
def build_full_market_underlying_top20_from_api5_pairs(api5_pairs, target_date):
    """
    用 API5 重新計算「全市場今日標的權證累積買超 TOP20」。
    ...
    計算邏輯：
      1. API4 只負責找出今日有成交權證與該權證當日出現的券商代號。
      2. 每一組「權證代號 + 券商代號」再用 API5 抓當日買進 / 賣出金額。
      3. 買超金額 = API5 買進金額 - API5 賣出金額。
      4. 預設只累積正買超，避免全市場買賣雙方互抵後趨近 0。
      5. 最後依標的股彙總並取前 20 名。

    這樣可以避免 API4 金額欄位單位判斷錯誤，造成不合理的榜單。
    """
    ranking_map = {}
    target_date = normalize_date_str(target_date)

    unique_pairs = {}
    for rec in api5_pairs:
        warrant_code = normalize_warrant_code_for_filter(rec.get("權證代號", ""))
        broker_code = str(rec.get("券商代號", "")).strip()

        if not warrant_code or not broker_code:
            continue

        key = (warrant_code, broker_code)
        if key not in unique_pairs:
            unique_pairs[key] = {
                "日期": target_date,
                "權證代號": warrant_code,
                "權證名稱": str(rec.get("權證名稱", "")).strip(),
    ...
    print(
        f"  🔎 用 API5 重算全市場標的買超 TOP20："
        f"{total:,} 組 權證×券商，日期 {target_date}，workers={FULL_MARKET_TOP20_API5_WORKERS}"
    )
    ...
    def fetch_one(rec):
        try:
            rows = api5_get(rec["權證代號"], rec["券商代號"])
            trade = extract_api5_trade_values_for_date(rows, target_date)

            if not trade:
                return None

            out = dict(rec)
            out["trade_values"] = trade
            return out
        except Exception:
            return None

    with ThreadPoolExecutor(max_workers=FULL_MARKET_TOP20_API5_WORKERS) as ex:
        futures = {ex.submit(fetch_one, rec): rec for rec in unique_records}
    ...
    rows = []
    for rec in ranking_map.values():
        if FULL_MARKET_TOP20_POSITIVE_ONLY and rec["累積買超金額"] <= 0:
            continue
    ...
    print(f"  ✅ API5 全市場標的買超 TOP20 重算完成：成功 API5 組合 {success:,} 組 → 榜單 {len(rows):,} 名")
    return rows
...
def build_full_market_underlying_top20_from_api4_rows(api4_rows, target_date):
    ...
    舊版曾直接使用 API4 欄位估算買超金額，但 API4 欄位順序 / 單位不穩，
    可能產生明顯不合理的大金額。新版改為：
      API4 只提供「權證代號 + 券商代號」候選組合；
      實際買進金額、賣出金額一律回 API5 重新計算。
    """
    return build_full_market_underlying_top20_from_api5_pairs(api4_rows, target_date)
...
def filter_warrants_by_today_volume(warrants):
    ...
    2. 同一次 API4 掃描中，同步建立「全市場今日標的權證累積買超 TOP20」。
       這張 TOP20 不使用 TARGET_PATTERNS、不追蹤特定分點、不顯示分點，且實際金額改由 API5 回查計算。
    """
    global ACTIVE_WARRANT_LATEST_TRADE_DATE, ACTIVE_WARRANT_FILTER_SOURCE
    global FULL_MARKET_UNDERLYING_TOP20_ROWS, FULL_MARKET_UNDERLYING_TOP20_DATE, FULL_MARKET_UNDERLYING_TOP20_SOURCE
    ...
    def scan_one(w):
        ...
        for row in rows:
            ...
            # API4 只作為今日有成交權證與當日分點券商代號的來源。
            # 金額欄位不再從 API4 解析，避免欄位單位判斷錯誤造成 TOP20 金額失真。
            broker_code = str(row.get("V2", "")).strip()
            if broker_code:
                ranking_rows.append({
                    "日期": date_str,
    ...
    FULL_MARKET_UNDERLYING_TOP20_DATE = latest_trade_date
    FULL_MARKET_UNDERLYING_TOP20_SOURCE = "api5_full_market_positive_buy"
    ...
    print(
        f"  ✅ 全市場標的買超 TOP20 建立完成：API4 來源權證分點列 {len(latest_api4_rows):,} 筆 "
        f"→ API5 重算後標的榜單 {len(top20_rows):,} 名"
    )
...
def main():
    ...
    print(f"今日有成交權證來源：ACTIVE_WARRANT_SOURCE={ACTIVE_WARRANT_SOURCE}（kgi=凱基今日成交量>0；moneydj_api4=舊版MoneyDJ全市場API4）")
    ...
    if ACTIVE_WARRANT_SOURCE == "kgi":
        # 新版主流程：不再先抓 ISIN 全市場權證清單，也不再用 MoneyDJ API4 掃全部權證。
        # 直接以 KGI 今日成交量 > 0 的認購權證作為唯一母體。
        warrants = get_active_call_warrants_from_kgi()

        if not warrants:
            print("⚠️ KGI 今日成交量 > 0 認購權證清單為空，停止執行。")
            elapsed = time.time() - program_start
            print(f"\n⏱️ 總執行時間：{elapsed:.2f} 秒")
            return

        build_full_market_underlying_top20_from_kgi_active_warrants(warrants)
    else:
        # 舊版備援流程，預設不使用。
