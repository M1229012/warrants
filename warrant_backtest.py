...
互斥規則：同一分點 + 同一檔權證代號只會出現在 A / B / C / D 其中一類，優先順序為 A > B > C > D；不同分點交易同一權證互不排除。
...
def build_b_events(daily_records, item_map):
    ...
    for key, g in df.groupby(group_cols):
        ...
        for wr in warrant_rows.itertuples(index=False):
            ...
            # B 類內部只排除同一分點已使用的同一檔權證；不同分點互不影響。
            if not warrant_key or warrant_key in used_b_warrant_keys:
                continue
        ...
        events.append(event)
        for lot in lots:
            warrant_key = make_broker_warrant_key(
                broker_code,
                lot["權證代號"],
            )
            if warrant_key:
                used_b_warrant_keys.add(warrant_key)
    ...
    for key, g in df.groupby(main_group_cols, sort=False):
        ...
        for start_idx in range(start_i_min, start_i_max + 1):
            ...
            for lot_key in sorted(window_lot_map.keys()):
                ...
                if not warrant_key or warrant_key in used_c_warrant_keys:
                    continue
            ...
            for lot in lots:
                ...
                if warrant_key:
                    used_c_warrant_keys.add(warrant_key)
    ...
    for key, g in df.groupby(main_group_cols, sort=False):
        ...
        for start_idx in range(start_i_min, start_i_max + 1):
            ...
            for warrant_code in sorted(window_warrant_map.keys()):
                ...
                # D 類內部只排除同一分點已使用的同一檔權證；不同分點互不影響。
                if not warrant_key or warrant_key in used_d_warrant_keys:
                    continue
            ...
            for lot in lots:
                ...
                if warrant_key:
                    used_d_warrant_keys.add(warrant_key)
