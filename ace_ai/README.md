...
本週精選綜合分數＝技術面 50＋權證面 50；權證面拆成「本次事件／精確複合事件歷史績效 20＋有效權證部位金額 15＋持倉／操作狀態 15」。勝率優先使用 Sheet 已存在的精確複合事件（例如 A+C）；找不到才退回單事件資料，分點總勝率只作背景資訊，不覆蓋高勝率事件。預設排除 2330（`WEEKLY_PICK_EXCLUDE_CODES`）。
...
4. `/ask 這版確認，生成圖片`：以最後確認文字產圖，K 線上改畫該篇涉及分點的實際權證淨買／淨賣點位。
...
`test_watermark_marks.py` 驗證長短圖浮水印、一般／族群／本週精選圖片出口，以及「沒有資料就不顯示診斷註解」；`test_weekly_v2.py` 另驗證 A+C 等複合事件正規化、精確複合事件優先，以及已出清舊事件不會被錯湊進目前事件組合。測試圖片明確使用合成行情，並非 3034 真實歷史價格；未連線讀取正式環境的分點事件或勝率資料。
