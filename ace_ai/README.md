...
本週精選完成後，在同一則訊息將「計算中」附件替換成結果圖片，不保留舊等待圖。`/ask` 編輯 deferred original response，`!ace` 保存等待訊息並編輯附件清單；計算例外同樣替換為錯誤圖片。私人回覆保留原本可見範圍。

若使用者手動刪除等待訊息，確認為 Unknown Message 後才重新發送；不會將失效的私人互動轉成公開頻道訊息。Discord 的 interaction token 有 15 分鐘有效期，若計算超時、Bot 重啟或 Discord 拒絕編輯，無法保證更新等待圖，會記錄傳送錯誤。參考 [discord.py Interactions API](https://discordpy.readthedocs.io/en/stable/interactions/api.html#discord.Interaction.edit_original_response)。

等待文字不再承諾 1～3 分鐘；首次查詢依資料量與上游速度可能需要數分鐘。更新成功後才印出 `Discord /ask 回覆圖片已更新` 或 `Discord !ace 回覆圖片已送出／更新`。計算完成紀錄與 Discord 傳送成功紀錄是不同階段。

這項更新對重新部署後的新查詢生效，不會回溯清除舊版本已留下的等待訊息。
...
共 31 項離線測試通過，涵蓋完整價量分布、布林突破／影線／重返通道、壓縮、擴張、橫盤、沿軌、資料不足、查詢路由、圖片傳送、等待附件替換、計算失敗、私人回覆和已刪除訊息。原有精選分數權重未新增布林分數；新增狀態提供給解讀。本週精選快取鍵已更新，避免沿用缺少布林欄位的舊結果。
