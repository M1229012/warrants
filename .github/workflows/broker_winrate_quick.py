...
# 0 或空白＝沿用回測設定；workflow 輸入欄被清空時會傳進空字串，不能直接 int()。
QUICK_DAYS = max(int(os.getenv("QUICK_DAYS", "0").strip() or "0"), 0)
...
def write_step_summary(result_df, failed_pairs):
    """GitHub Actions 上把勝率表寫進 Summary 頁；本機沒有這個 env 就略過。"""
    summary_path = os.getenv("GITHUB_STEP_SUMMARY", "").strip()
    if not summary_path or result_df.empty:
        return
    lines = []
    for (name, code), g in result_df.groupby(["分點名稱", "券商代號"], sort=False):
        lines += [
            f"### {name}（{code}）｜統計日 {g['統計日期'].iloc[0]}",
            "",
            "| 類別 | 事件 | 納入 | 勝/敗/平 | 勝率 | 平均報酬 | 加權報酬 | 均持有 |",
            "|---|---:|---:|---:|---:|---:|---:|---:|",
        ]
        for s in g.to_dict("records"):
            hold = "-" if pd.isna(s["平均持有天數"]) else f"{s['平均持有天數']:.0f}天"
            lines.append(
                f"| {s['事件類型']} | {s['事件數']} | {s['納入勝率筆數']} "
                f"| {s['勝筆數']}/{s['敗筆數']}/{s['平手筆數']} "
                f"| {_fmt_pct(None if pd.isna(s['勝率']) else s['勝率'])} "
                f"| {_fmt_pct(None if pd.isna(s['平均報酬%']) else s['平均報酬%'], True)} "
                f"| {_fmt_pct(None if pd.isna(s['加權報酬%']) else s['加權報酬%'], True)} | {hold} |"
            )
        lines.append("")
    if failed_pairs:
        lines.append(f"> ⚠️ API5 仍有 {failed_pairs:,} 組取不到，結果不完整。")
...
def run_quick_winrate():
    ...
    write_step_summary(result_df, failed_pairs)
