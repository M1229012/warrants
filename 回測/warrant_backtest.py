...
互斥規則：同一分點 + 同一檔權證代號只會出現在 A / B / C / D 其中一類，優先順序為 A > B > C > D；不同分點交易同一權證互不排除。每次符合條件的事件各自成一筆，後續賣出依剩餘股數 FIFO 扣減；金額僅用於成本與損益。
...
# 結果同步前是否掃描工作表並縮減空白格。
# write_values_to_worksheet() 寫入時本來就會把本次工作表 resize 成實際大小，
# 因此預設關閉同步前完整掃描，避免大型 Google Sheet 重複讀取所有工作表內容。
# 需要手動整理未被本次流程寫入的其他工作表時，可設為 1。
GSHEET_COMPACT_BEFORE_RESULT_SYNC = os.getenv(
    "GSHEET_COMPACT_BEFORE_RESULT_SYNC",
    "0",
...
# Google Sheet 舊分點資料清理：
# 每次執行時只檢查「有直接分點欄位」的工作表，將不在 FULL_TARGET_PATTERNS / FULL_FALLBACK
# 中的舊分點資料列刪除。只刪除資料列，絕不刪除整張工作表。
# 採兩階段批次讀取：先批次讀前幾列表頭，再只讀分點／券商代號欄，避免下載整張大型快取。
GSHEET_PRUNE_UNKNOWN_BROKER_ROWS_ENABLED = os.getenv(
    "GSHEET_PRUNE_UNKNOWN_BROKER_ROWS_ENABLED",
    "1",
).strip().lower() not in ("0", "false", "no")
GSHEET_PRUNE_UNKNOWN_BROKER_HEADER_SCAN_ROWS = max(
    int(os.getenv("GSHEET_PRUNE_UNKNOWN_BROKER_HEADER_SCAN_ROWS", "10")),
    1,
)
...
# 同一張結果工作表的格式請求暫存區。
# 啟用時，各格式函式只收集 request，最後一次送出 batchUpdate，
# 避免每個格式函式各自觸發一次節流與 API 寫入。
_GSHEET_BATCH_REQUEST_BUFFER = None
_GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE = 1000
_GSHEET_BATCH_REQUEST_BUFFER_LABEL = ""
...
_GSHEET_BROKER_BLOCK_TITLES = {
    "勝率統計",
    "ABCD組合勝率",
}

_GSHEET_NON_BROKER_ROW_MARKERS = {
    "",
    "-",
    "--",
    "---",
...
def _quote_gsheet_a1_title(title):
    """將工作表名稱安全轉成 A1 notation 可用的單引號格式。"""
    return "'" + str(title).replace("'", "''") + "'"


def _normalize_broker_identity_text(value):
    ...
    s = re.sub(r"[\s\-－_—–・．\.]+", "", s)
    s = s.replace("證券股份有限公司", "證券")
    s = s.replace("股份有限公司", "")
    return s.upper()


def _configured_broker_identity_sets():
    labels = {
        _normalize_broker_identity_text(label)
        for label in FULL_TARGET_PATTERNS.keys()
        if _normalize_broker_identity_text(label)
    }
    names = {
        _normalize_broker_identity_text(name)
        for name, _ in FULL_FALLBACK.values()
        if _normalize_broker_identity_text(name)
    }
    codes = {
        str(code).strip().upper()
        for _, code in FULL_FALLBACK.values()
        if str(code).strip()
    }
    return labels | names, codes


def _broker_text_matches_full_config(value, allowed_names=None):
    ...
    normalized = _normalize_broker_identity_text(raw)
    if not normalized:
        return False

    if allowed_names is None:
        allowed_names, _ = _configured_broker_identity_sets()

    if normalized in allowed_names:
        return True

    for pattern in FULL_TARGET_PATTERNS.values():
        try:
...
def _broker_code_matches_full_config(value, allowed_codes=None):
    raw = strip_gsheet_text_prefix(value)
    raw = str(raw or "").strip()
    ...
    if raw.endswith(".0") and raw[:-2].isalnum():
        raw = raw[:-2]

    if allowed_codes is None:
        _, allowed_codes = _configured_broker_identity_sets()

    return raw.upper() in allowed_codes


def _is_non_broker_marker(value):
    raw = strip_gsheet_text_prefix(value)
    raw = str(raw or "").strip()
    ...
    return raw.upper() in _GSHEET_NON_BROKER_ROW_MARKERS


def _row_has_configured_broker(identity_by_header, allowed_names, allowed_codes):
    for header, value in identity_by_header.items():
        header = str(header).strip()
        if header in ("券商代號", "券商代碼"):
            if _broker_code_matches_full_config(value, allowed_codes):
                return True
        elif _broker_text_matches_full_config(value, allowed_names):
            return True
    return False


def _row_is_unknown_broker(identity_by_header, allowed_names, allowed_codes):
    non_empty = {
        header: value
        for header, value in identity_by_header.items()
        if str(strip_gsheet_text_prefix(value) or "").strip()
    }

    if not non_empty:
        return False

    # 公式、合計、總計等非分點資料列保留，避免誤刪報表說明列。
    if any(str(strip_gsheet_text_prefix(value) or "").strip().startswith("=") for value in non_empty.values()):
        return False

    if all(_is_non_broker_marker(value) for value in non_empty.values()):
        return False

    # 重複表頭列不視為資料列。
    if all(str(value).strip() == str(header).strip() for header, value in non_empty.items()):
        return False

    return not _row_has_configured_broker(non_empty, allowed_names, allowed_codes)


def _contiguous_row_ranges(row_numbers):
    """把 1-based 列號壓縮成連續區間，回傳 [(start_row, end_row_exclusive), ...]。"""
    rows = sorted({int(row) for row in row_numbers if int(row) >= 1})
    if not rows:
        return []
...
def _gsheet_values_batch_get(spreadsheet, ranges):
    """批次讀取多個 A1 範圍；失敗時回傳與 ranges 等長的空清單。"""
    if spreadsheet is None or not ranges:
    ...
    try:
        ...
        for idx in range(len(ranges)):
            item = value_ranges[idx] if idx < len(value_ranges) else {}
            out.append(item.get("values", []) if isinstance(item, dict) else [])
        return out
    except TypeError:
        try:
            payload = spreadsheet.values_batch_get(ranges)
            value_ranges = payload.get("valueRanges", []) if isinstance(payload, dict) else []
            out = []
            for idx in range(len(ranges)):
                item = value_ranges[idx] if idx < len(value_ranges) else {}
...
def _find_direct_broker_header_row(header_values):
    """在前幾列找出直接分點欄位，回傳 (1-based header row, [(0-based col, header), ...])。"""
    for row_idx, row in enumerate(header_values or [], start=1):
        direct_cols = []
        for col_idx, raw_header in enumerate(list(row)):
            header = str(raw_header or "").strip()
            if header in _GSHEET_DIRECT_BROKER_HEADERS:
                direct_cols.append((col_idx, header))

        if direct_cols:
            return row_idx, direct_cols

    return None, []


def _unknown_broker_block_delete_ranges(column_a_values, allowed_names):
    """勝率統計類工作表：依「分點：xxx」標題刪除整個未知分點區塊。"""
    title_rows = []

    for row_idx, row in enumerate(column_a_values or [], start=1):
        value = str(row[0] if row else "").strip()
        match = re.match(r"^分點\s*[:：]\s*(.+?)\s*$", value)
        if match:
            title_rows.append((row_idx, match.group(1).strip()))

    if not title_rows:
        return []

    last_used_row = len(column_a_values)
    delete_ranges = []

    for idx, (start_row, broker_name) in enumerate(title_rows):
        end_row_exclusive = title_rows[idx + 1][0] if idx + 1 < len(title_rows) else last_used_row + 1
        if not _broker_text_matches_full_config(broker_name, allowed_names):
            delete_ranges.append((start_row, end_row_exclusive))

    return delete_ranges


def prune_unknown_broker_rows_from_gsheet(spreadsheet=None):
    """
    刪除 Google Sheet 中已不在程式碼完整分點清單內的舊資料列。

    安全規則：
    1. 只刪資料列，從不呼叫 del_worksheet()。
    2. 允許清單永遠使用 FULL_TARGET_PATTERNS / FULL_FALLBACK，與 RUN_MODE 無關；
       RUN_MODE=1 執行時不會把其餘仍在完整程式清單中的分點誤刪。
    3. 先批次讀取各工作表前幾列表頭，再只讀直接分點欄位，避免下載整張大型歷史快取。
    4. 勝率統計與 ABCD 組合勝率依分點區塊整段刪除，避免留下空標題與表頭。
    """
    if not GSHEET_PRUNE_UNKNOWN_BROKER_ROWS_ENABLED:
        return 0
    ...
    try:
        ...
        print(f"  ⚠️ Google Sheet 舊分點資料檢查失敗：{type(e).__name__}: {e}")
        return 0

    if not worksheets:
        return 0

    allowed_names, allowed_codes = _configured_broker_identity_sets()
    header_scan_rows = GSHEET_PRUNE_UNKNOWN_BROKER_HEADER_SCAN_ROWS

    # 第一階段：所有工作表只讀前幾列，成本小，可自動發現任何含直接分點欄位的表。
    header_ranges = []
    header_ws = []
    for ws in worksheets:
        max_col = min(max(int(getattr(ws, "col_count", 1) or 1), 1), 702)
        end_col = get_column_letter(max_col)
        header_ranges.append(
            f"{_quote_gsheet_a1_title(ws.title)}!A1:{end_col}{header_scan_rows}"
        )
        header_ws.append(ws)

    header_results = _gsheet_values_batch_get(spreadsheet, header_ranges)
    sheet_plans = []
    block_plans = []

    for ws, header_values in zip(header_ws, header_results):
        title = safe_worksheet_title(getattr(ws, "title", ""))

        if title in _GSHEET_BROKER_BLOCK_TITLES:
            block_plans.append(ws)
            continue

        header_row, direct_cols = _find_direct_broker_header_row(header_values)
        if header_row is None or not direct_cols:
            continue

        sheet_plans.append((ws, header_row, direct_cols))

    # 第二階段：只讀需要的分點欄位；大型快取不再把金額、日期等無關欄位下載回來。
    column_ranges = []
    column_specs = []

    for ws, header_row, direct_cols in sheet_plans:
        row_count = max(int(getattr(ws, "row_count", header_row) or header_row), header_row)
        if row_count <= header_row:
            continue

        for col_idx, header in direct_cols:
            col_letter = get_column_letter(col_idx + 1)
            column_ranges.append(
                f"{_quote_gsheet_a1_title(ws.title)}!{col_letter}{header_row + 1}:{col_letter}{row_count}"
            )
            column_specs.append((ws, header_row, col_idx, header))

    column_results = _gsheet_values_batch_get(spreadsheet, column_ranges)
    sheet_identity_columns = {}

    for spec, values in zip(column_specs, column_results):
        ws, header_row, col_idx, header = spec
        key = int(ws.id)
        sheet_identity_columns.setdefault(key, {
            "ws": ws,
            "header_row": header_row,
            "columns": {},
        })
        sheet_identity_columns[key]["columns"][header] = values or []

    delete_ranges_by_sheet = {}

    for plan in sheet_identity_columns.values():
        ws = plan["ws"]
        header_row = int(plan["header_row"])
        ...
        for offset in range(max_len):
            ...
            for header, values in columns.items():
                ...
                identity[header] = row[0] if row else ""

            if _row_is_unknown_broker(identity, allowed_names, allowed_codes):
                delete_rows.append(header_row + 1 + offset)

        if delete_rows:
            delete_ranges_by_sheet[int(ws.id)] = {
                "ws": ws,
                "ranges": _contiguous_row_ranges(delete_rows),
            }

    # 多區塊工作表只讀 A 欄，依「分點：xxx」標題一次刪除完整區塊。
    block_ranges = []
    for ws in block_plans:
        row_count = max(int(getattr(ws, "row_count", 1) or 1), 1)
        block_ranges.append(f"{_quote_gsheet_a1_title(ws.title)}!A1:A{row_count}")

    block_results = _gsheet_values_batch_get(spreadsheet, block_ranges)
    for ws, values in zip(block_plans, block_results):
        ranges = _unknown_broker_block_delete_ranges(values, allowed_names)
        if ranges:
            delete_ranges_by_sheet[int(ws.id)] = {
                "ws": ws,
                "ranges": ranges,
    ...
    for sheet_id, plan in delete_ranges_by_sheet.items():
        ranges = sorted(plan["ranges"], key=lambda x: x[0], reverse=True)
        if not ranges:
            continue
        ...
        for start_row, end_row_exclusive in ranges:
            count = max(int(end_row_exclusive) - int(start_row), 0)
            if count <= 0:
                continue

            # Google Sheets API 使用 0-based、end-exclusive；start_row / end_row_exclusive 是 1-based。
            requests.append({
                "deleteDimension": {
                    "range": {
                        "sheetId": int(sheet_id),
                        "dimension": "ROWS",
                        "startIndex": int(start_row) - 1,
                        "endIndex": int(end_row_exclusive) - 1,
                    }
                }
            })
            sheet_removed += count
        ...
        if sheet_removed > 0:
            print(
                f"  🧹 Google Sheet 已標記刪除舊分點資料：{plan['ws'].title}｜{sheet_removed:,} 列"
            )

    if not requests:
        print("  ✅ Google Sheet 舊分點資料檢查完成：沒有發現程式碼清單外的分點資料")
        return 0

    # 同一工作表必須由下往上刪，避免列號位移；requests 已依各工作表倒序建立。
    _gsheet_batch_update(requests, chunk_size=500)
    print(
        f"  🧹 Google Sheet 舊分點資料清理完成：{affected_sheets} 張工作表｜"
        f"刪除 {removed_rows:,} 列｜未刪除任何工作表"
    )
    return removed_rows
...
def write_values_to_worksheet(ws, values, apply_text_format=True):
    if ws is None:
        return False
    ...
    try:
        ...
        target_rows = max(row_count, 1)
        target_cols = max(col_count, 1)
        current_rows = max(int(getattr(ws, "row_count", 1) or 1), 1)
        current_cols = max(int(getattr(ws, "col_count", 1) or 1), 1)

        # 尺寸相同時略過 resize，可少一次 Google Sheets 寫入請求與節流等待。
...
def _dedupe_gsheet_requests(requests):
    """只移除完全相同的 Google Sheets request，保留原順序與覆蓋先後。"""
    out = []
    seen = set()
...
def _send_gsheet_batch_update(requests, chunk_size=1000, label="", dedupe=False):
    if dedupe:
        requests = _dedupe_gsheet_requests(requests)
    else:
    ...
    label_text = f" {label}" if str(label or "").strip() else ""

    for start in range(0, len(requests), chunk_size):
        chunk = requests[start:start + chunk_size]
        try:
            gsheet_api_call(
                f"套用 Google Sheet 格式{label_text} batchUpdate {start + 1}-{start + len(chunk)}",
                sh.batch_update,
                {"requests": chunk},
            )
        except Exception as e:
            print(f"  ⚠️ Google Sheet 格式套用失敗{label_text}：{type(e).__name__}: {e}")
            return


def _gsheet_batch_update(requests, chunk_size=1000):
    """
    一般情況直接送出 batchUpdate；若目前位於 gsheet_batch_update_group()，
    則先累積 requests，待同一張工作表所有格式都準備完成後一次送出。
    """
    global _GSHEET_BATCH_REQUEST_BUFFER
    global _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE

    if not requests:
        return

    if _GSHEET_BATCH_REQUEST_BUFFER is not None:
        _GSHEET_BATCH_REQUEST_BUFFER.extend(list(requests))
        _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE = min(
            max(int(_GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE or 1000), 1),
            max(int(chunk_size or 1000), 1),
        )
        return

    _send_gsheet_batch_update(requests, chunk_size=chunk_size, dedupe=False)


@contextmanager
def gsheet_batch_update_group(label="", chunk_size=1000):
    """
    將同一區段內所有 _gsheet_batch_update() 合併後送出。

    支援巢狀呼叫：內層沿用外層暫存區，不會提前送出。
    """
    global _GSHEET_BATCH_REQUEST_BUFFER
    global _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE
    global _GSHEET_BATCH_REQUEST_BUFFER_LABEL

    if _GSHEET_BATCH_REQUEST_BUFFER is not None:
        yield
        return

    _GSHEET_BATCH_REQUEST_BUFFER = []
    _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE = max(int(chunk_size or 1000), 1)
    _GSHEET_BATCH_REQUEST_BUFFER_LABEL = str(label or "").strip()
    caught_error = None
    ...
    try:
        ...
        buffered = _GSHEET_BATCH_REQUEST_BUFFER
        buffered_chunk_size = _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE
        buffered_label = _GSHEET_BATCH_REQUEST_BUFFER_LABEL
        _GSHEET_BATCH_REQUEST_BUFFER = None
        _GSHEET_BATCH_REQUEST_BUFFER_CHUNK_SIZE = 1000
        _GSHEET_BATCH_REQUEST_BUFFER_LABEL = ""

    # 即使某個格式產生步驟失敗，先送出已完成的安全 requests，再把原錯誤往上拋。
    _send_gsheet_batch_update(
        buffered,
        chunk_size=buffered_chunk_size,
...
def upload_excel_to_google_sheet(xlsx_path):
    ...
    try:
        ...
        if primary_sh is not None:
            ...
            if GSHEET_COMPACT_BEFORE_RESULT_SYNC:
                ...
                print(
                    "  ⚡ 已略過 Google Sheet 同步前空白格完整掃描；"
                    "本次寫入工作表會直接 resize 成實際大小。"
                )
        ...
        for ws_xlsx in wb.worksheets:
            ...
            # 結果工作表的文字 / 數字 / 日期 / 配色 / 欄寬統一在下方批次處理，
            # 因此寫值階段不先單獨送一次 TEXT 格式 request。
            if write_values_to_worksheet(gws, values, apply_text_format=False):
                with gsheet_batch_update_group(label=title, chunk_size=1000):
...
def build_a_events_from_df(item):
    """
    A 類逐筆事件 FIFO：
    1. 每一個「單日買進金額 >= AMOUNT_THRESH」的買進日，獨立建立一筆 A 事件。
    2. 同一天若同時有買進與賣出，先用賣出股數扣除較早買進事件，再建立當日新事件。
    3. 同一分點 + 同一權證的賣出，依買進日由早到晚 FIFO 扣減。
    4. 減碼 / 出清只依每筆事件的剩餘股數判斷；成交金額只用於計算實現損益。
    """
    df = item["df"]
    events = []
    ...
    for row in df.itertuples(index=False):
        row_dict = row._asdict()
        date   = normalize_date_str(row_dict["日期"])
        buy_s  = int(row_dict["買進股數"])
        sell_s = int(row_dict["賣出股數"])
        buy_a  = int(row_dict["買進金額"])
        sell_a = int(row_dict["賣出金額"])

        # 權證不可當沖：同一天先賣舊事件，再建立當日新買進事件。
        if sell_s > 0 and events:
            sell_price = (sell_a / sell_s) if sell_s > 0 else 0
            sell_left = sell_s
            sell_dt = parse_date(date)

            # events 本身即依買進日建立；再次明確排序，確保 FIFO 不受其他流程影響。
            fifo_events = sorted(
                events,
                key=lambda ev: (
                    parse_date(ev.get("買進日")) or datetime.max,
                    int(ev.get("事件序號", 0) or 0),
                ),
            )
            ...
            for ev in fifo_events:
                ...
                remaining_before = int(ev.get("剩餘股數", 0) or 0)
                if remaining_before <= 0:
                    continue

                ev_buy_dt = parse_date(ev.get("買進日"))
                if ev_buy_dt and sell_dt and ev_buy_dt >= sell_dt:
                    continue

                alloc = min(sell_left, remaining_before)
                if alloc <= 0:
                    continue

                alloc_revenue = alloc * sell_price
                alloc_cost = alloc * float(ev.get("買進均價", 0) or 0)

                ev["剩餘股數"] = remaining_before - alloc
                ev["累計賣出股數"] = int(ev.get("累計賣出股數", 0) or 0) + alloc
                ev["已實現賣出金額"] = float(ev.get("已實現賣出金額", 0) or 0) + alloc_revenue
                ev["已實現成本"] = float(ev.get("已實現成本", 0) or 0) + alloc_cost
                sell_left -= alloc

                # 狀態只看剩餘股數，不能用賣出金額是否高於買進成本判斷。
                if ev["剩餘股數"] > 0:
                    if ev.get("減碼日") is None:
                        ev["減碼日"] = date
                        ev["減碼均價"] = round(sell_price, 4)
                        ev["減碼獲利%"] = round(
                            (alloc_revenue - alloc_cost) / alloc_cost * 100,
                            2,
                        ) if alloc_cost else None
                    ev["狀態"] = "減碼"
                else:
                    ev["剩餘股數"] = 0
                    ev["狀態"] = "出清"
                    ...
                    total_sold_shares = int(ev.get("累計賣出股數", 0) or 0)
                    realized_revenue = float(ev.get("已實現賣出金額", 0) or 0)
                    cost_basis = float(ev.get("買進金額", 0) or 0)

                    ev["出清均價"] = round(
                        realized_revenue / total_sold_shares,
                        4,
                    ) if total_sold_shares > 0 else round(sell_price, 4)

                    ev["出清獲利%"] = round(
                        (realized_revenue - cost_basis) / cost_basis * 100,
                        2,
                    ) if cost_basis else None

                    buy_dt = parse_date(ev.get("買進日"))
                    exit_dt = parse_date(date)
                    if buy_dt and exit_dt:
                        ev["持有天數"] = (exit_dt - buy_dt).days

        # 每次達到 A 門檻的買進都獨立成一筆，不與前一筆事件合併。
...
def normalize_warrant_code_for_unique(warrant_code):
    """
    用於 A / B / C / D 權證互斥判斷的權證代號正規化。

    權證互斥不能只看權證代號，必須再搭配券商代號，
    否則不同分點交易同一檔權證時會互相覆蓋。
    """
...
def make_broker_warrant_key(broker_code, warrant_code):
    """
    建立 A / B / C / D 分類與互斥使用的唯一鍵。

    正確單位為「券商代號 + 權證代號」：
    - 同一分點交易同一權證，只能歸入 A / B / C / D 其中一類。
    - 不同分點即使交易相同權證，也必須分開保留，不能互相排除。
    """
...
def collect_event_warrant_keys(events):
    """
    從事件清單收集已使用的「券商代號 + 權證代號」。

    A 類事件：直接讀取事件本身的權證代號。
    B / C / D 類事件：使用事件的券商代號，搭配 lots 內每一檔權證代號。
    """
    keys = set()
    ...
    for ev in events or []:
        ...
        direct_key = make_broker_warrant_key(
            broker_code,
            ev.get("權證代號", ""),
        )
        if direct_key:
        ...
        for lot in ev.get("lots", []):
            lot_key = make_broker_warrant_key(
                broker_code,
                lot.get("權證代號", ""),
            )
            if lot_key:
...
def filter_daily_records_by_warrant_keys(daily_records, exclude_warrant_keys):
    """
    依「券商代號 + 權證代號」過濾較低優先級事件的原始資料。

    只排除同一分點中已進入較高優先級事件的權證；
    其他分點交易相同權證時仍會保留，避免跨分點資料被錯誤刪除。
    """
    if not exclude_warrant_keys:
        return daily_records

    exclude_warrant_keys = {
        key
        for key in exclude_warrant_keys
        if key
    }
    ...
    for row in daily_records:
        row_key = make_broker_warrant_key(
            row.get("券商代號", ""),
            row.get("權證代號", ""),
        )

        if row_key not in exclude_warrant_keys:
            filtered.append(row)
...
def filter_a_events_unique_warrants(a_events):
    ...
    唯一鍵為「券商代號 + 權證代號 + 買進日」：
    1. 不同分點交易同一權證，分開保留。
    2. 同一分點、同一權證在不同日期達到 A 門檻，分開保留。
    3. 只有同一分點、同一權證、同一買進日重複時，才保留買進金額較大的那筆。

    A / B / C / D 類別互斥仍由 collect_event_warrant_keys() 以
    「券商代號 + 權證代號」處理，不會讓同一交易對象重複進入其他類別。
    """
    if not a_events:
        return []
    ...
    for ev in sorted_events:
        broker_warrant_key = make_broker_warrant_key(
            ev.get("券商代號", ""),
            ev.get("權證代號", ""),
        )
        buy_date = normalize_date_str(ev.get("買進日") or ev.get("事件日") or "")

        if not broker_warrant_key or not buy_date:
            continue

        event_key = broker_warrant_key + (buy_date,)
...
def simulate_group_outcomes_fifo(events, item_map):
    """
    對同一類別的 B / C / D 全部事件進行「跨事件、逐筆 FIFO」扣減。
...
def build_b_events(daily_records, item_map):
    ...
    return simulate_group_outcomes_fifo(events, item_map)
    ...
    for key, g in df.groupby(main_group_cols, sort=False):
        ...
        for start_idx in range(start_i_min, start_i_max + 1):
            ...
            for warrant_code in sorted(window_warrant_map.keys()):
                ...
                warrant_code_norm = normalize_warrant_code_for_unique(warrant_code)
                warrant_key = make_broker_warrant_key(broker_code, warrant_code_norm)
...
def collect_top15_return_recent_dates(
    ...
    for ev in a_events or []:
        dt = parse_date(ev.get("買進日") or ev.get("事件日"))
        if dt and dt <= target_dt:
            dates.add(dt.strftime("%Y/%m/%d"))
...
def collect_top15_return_position_lots(a_events, b_events, c_events, d_events, recent_dates, item_map=None):
    """
    將 40 個市場交易日範圍內的 A/B/C/D 事件標記成 TOP15 事件 lot。

    同一筆「券商代號＋權證代號＋實際買進日」原始流水只建立一次；
    若 C／D 視窗重疊指向同一買進列，只合併事件來源，不重複增加股數與成本。
    """
    ...
    for ev in a_events or []:
        ...
        item = _top15_find_item(item_map, ev.get("券商代號", ""), ev.get("權證代號", ""))
        used_history = False
        if item:
            df = item.get("df", pd.DataFrame())
            if df is not None and not df.empty:
                df2 = df.copy()
                df2["日期"] = df2["日期"].map(normalize_date_str)
                matched = df2[df2["日期"] == event_date]
                if not matched.empty:
                    row = matched.iloc[-1]
                    used_history = add_lot(
                        "A", ev.get("事件類型", "A-單檔權證大買"),
                        ev.get("分點", ""), ev.get("分點名稱", ""), ev.get("券商代號", ""),
                        ev.get("標的股", ""), ev.get("標的名稱", ""),
                        event_date, event_date, ev.get("權證代號", ""), ev.get("權證名稱", ""),
                        row.get("買進金額", 0), row.get("買進股數", 0),
                        f'A | {event_date} | {ev.get("權證代號", "")} {ev.get("權證名稱", "")}',
                    )
        if not used_history:
            add_lot(
                "A", ev.get("事件類型", "A-單檔權證大買"),
...
def _top15_lp_accept_disclaimer(session, base_url):
    try:
        ...
        response = session.get(
            root_url,
            headers={"User-Agent": HDR.get("User-Agent", "Mozilla/5.0")},
...
def build_top15_position_detail_and_consensus_rows(a_events, b_events, c_events, d_events, item_map, price_cache, target_date=None):
    ...
    # 僅針對統計日沒有成交收盤價的權證查詢 LP 報價；有當日成交價者不會查平台。
    lp_quote_needed_codes = set()
    for lot in position_lots:
        code = normalize_price_code(lot.get("權證代號", ""))
        if not code:
            continue
        traded_price, traded_price_date = get_latest_price_info_on_or_before(price_cache, code, target_date)
...
def build_price_prefetch_context_from_items(items):
    ...
    a_warrant_keys = collect_event_warrant_keys(a_events)
    daily_records_for_b = filter_daily_records_by_warrant_keys(daily_records, a_warrant_keys)

    b_events = build_b_events(daily_records_for_b, item_map)

    b_warrant_keys = collect_event_warrant_keys(b_events)
    daily_records_for_c = filter_daily_records_by_warrant_keys(
        daily_records_for_b,
        b_warrant_keys,
    )
    ...
    c_warrant_keys = collect_event_warrant_keys(c_events)
    daily_records_for_d = filter_daily_records_by_warrant_keys(
        daily_records_for_c,
        c_warrant_keys,
    )
...
def main():
    ...
    # 每次執行都先檢查 Google Sheet：只刪除不在完整程式分點清單內的舊資料列。
    # 此步驟與 RUN_MODE 無關，不會把完整清單內、但本次未追蹤的分點刪除；也絕不刪除工作表。
    prune_unknown_broker_rows_from_gsheet()
    ...
    # A 類只移除同一分點、同一權證、同一買進日的真正重複事件；
    # 不同分點或不同買進日的 A 事件都必須保留，供 TOP15 正確累計部位。
    a_events = filter_a_events_unique_warrants(a_events)

    daily_records = build_daily_records(items)

    # A > B > C > D 以「券商代號 + 權證代號」互斥：
    # 同一分點的同一檔權證進入 A 後，不再進入 B / C / D；不同分點互不排除。
    a_warrant_keys = collect_event_warrant_keys(a_events)
    daily_records_for_b = filter_daily_records_by_warrant_keys(daily_records, a_warrant_keys)
