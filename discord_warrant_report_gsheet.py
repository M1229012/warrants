...
def draw_broker_10d_detail_image(target: date, broker: str, output_path: Path):
    ...
    summary_rows = 2
    table_h = table_title_h + header_h + max(1, len(rows)) * row_h
    fig_h = top_h + summary_rows * summary_card_h + (summary_rows - 1) * summary_gap + gap + table_h + footer_h + 0.40
    fig_h = max(fig_h, 9.2)
    ...
    def draw_clipped_text(x_left, y_center, cell_w, text_value, size=13.0, color=TEXT, fp=None, padding_left=0.12, padding_right=0.12):
        display_text = fit_to_cell_width(text_value, cell_w - padding_left - padding_right, size=size, fp=fp)
        clip_rect = patches.Rectangle((x_left, y_center - row_h / 2 + 0.02), cell_w, row_h - 0.04, linewidth=0, facecolor="none")
        ax.add_patch(clip_rect)
        t = ax.text(
    ...
    text_draw(
        margin_x + 0.18,
        y,
        f"統計期間：{period_text}｜統計日期：{cache_date_text}｜同標的全部權證合併統計｜單位：元 / 報酬率：%",
        12.6,
        TEXT,
        BOLD,
    )
    ...
    summary_cards1 = [
        ("近10日買進金額", f"{safe_float(meta.get('buy_total'),0):,.0f}", RED),
        ("近10日賣出金額", f"{safe_float(meta.get('sell_total'),0):,.0f}", GREEN),
        ("近10日淨買賣超", f"{safe_float(meta.get('net_total'),0):,.0f}", NAVY2),
    ]
    for i, (label, value, color) in enumerate(summary_cards1):
        x = margin_x + i * (card_w + card_gap_x)
        rounded(x, card_y1, card_w, summary_card_h, fc=WHITE, ec=BORDER, lw=1.0, r=0.08)
        text_draw(x + 0.18, card_y1 + summary_card_h - 0.24, label, 12.5, MUTED, BOLD, va="top")
        text_draw(x + 0.18, card_y1 + 0.30, value, 18, color, BOLD, va="bottom")

    def fmt_pct_no_plus(v):
        if v is None:
            return "-"
        pct = safe_float(v, None)
        if pct is None:
    ...
    summary_cards2 = [
        ("買超平均報酬", fmt_return_pct(meta.get("avg_buy_return")), RED),
        ("賣超平均報酬", fmt_return_pct(meta.get("avg_sell_return")), GREEN),
        ("近10日勝率", fmt_pct_no_plus(meta.get("win_rate")), ORANGE),
    ]
    for i, (label, value, color) in enumerate(summary_cards2):
        x = margin_x + i * (card_w + card_gap_x)
        rounded(x, card_y2, card_w, summary_card_h, fc=WHITE, ec=BORDER, lw=1.0, r=0.08)
        text_draw(x + 0.18, card_y2 + summary_card_h - 0.24, label, 12.5, MUTED, BOLD, va="top")
        text_draw(x + 0.18, card_y2 + 0.30, value, 18, color, BOLD, va="bottom")
        if i == 2:
            text_draw(x + card_w - 0.18, card_y2 + 0.22, f"勝 {safe_int(meta.get('win_count'),0)} / 敗 {safe_int(meta.get('loss_count'),0)}", 11.4, TEXT, FONT, ha="right")

    y = card_y2 - gap

    rounded(margin_x, y - table_h, content_w, table_h, fc=WHITE, ec=NAVY, lw=1.2, r=0.08)
    rect(margin_x, y - table_title_h, content_w, table_title_h, fc=NAVY)
    text_draw(margin_x + 0.28, y - table_title_h / 2, f"{broker}｜近10日標的買賣明細（共 {len(rows)} 檔）", 18, WHITE, BOLD)

    headers = ["排名", "標的", "方向", "買進金額", "賣出金額", "淨額", "買超報酬", "賣超報酬", "權證數"]
    col_w = [0.55, 2.85, 0.72, 1.55, 1.55, 1.50, 1.12, 1.12, 0.90]
    header_y_top = y - table_title_h
    rect(margin_x, header_y_top - header_h, content_w, header_h, fc=HEADER_BG, ec=BORDER, lw=0.6)

    x = margin_x
    for h, w in zip(headers, col_w):
        text_draw(x + w / 2, header_y_top - header_h / 2, h, 11.8, NAVY, BOLD, ha="center")
        ax.plot([x, x], [y - table_h, header_y_top], color=BORDER, linewidth=0.6)
        x += w
    ax.plot([margin_x + content_w, margin_x + content_w], [y - table_h, header_y_top], color=BORDER, linewidth=0.6)
    ...
    if not rows:
        ...
        for i, r in enumerate(rows, start=1):
            ...
            direction_color = RED if direction == "買超" else GREEN if direction == "賣超" else MUTED
            net_value = safe_float(r.get("net_buy_amount"), 0) if direction == "買超" else -safe_float(r.get("net_sell_amount"), 0) if direction == "賣超" else safe_float(r.get("buy_amount"), 0) - safe_float(r.get("sell_amount"), 0)
            net_text = f"{net_value:,.0f}"
            net_color = RED if net_value > 0 else GREEN if net_value < 0 else TEXT
            ...
            values = [
                ...
                net_text,
                fmt_return_pct(r.get("buy_return")),
                fmt_return_pct(r.get("sell_return")),
                str(safe_int(r.get("warrant_count"), 0)),
            ]
            colors = [TEXT, TEXT, direction_color, RED, GREEN, net_color, RED, GREEN, TEXT]
            aligns = ["center", "left", "center", "right", "right", "right", "right", "right", "center"]
            bolds = [True, True, True, True, True, True, False, False, True]
            ...
            for val, w, c, a, is_bold in zip(values, col_w, colors, aligns, bolds):
                ...
                if a == "left":
                    ...
                    display_val = fit_to_cell_width(val, max(0.2, w - 0.24), size=12.6, fp=fp)
                    px = x + (w / 2 if a == "center" else w - 0.12)
                    text_draw(px, ry + row_h / 2, display_val, 12.6, c, fp, ha=a)
                x += w

    text_draw(fig_w / 2, 0.18, "本圖為近10日分點買賣明細整理，不構成投資建議。", 11, MUTED, FONT, ha="center")
